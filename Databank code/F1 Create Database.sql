...
CREATE TABLE Race (
    ...
    YearOf1GP INT NOT NULL,
    fk_Result1ID INT NOT NULL,
    fk_Result2ID INT NOT NULL,
    fk_Result3ID INT NOT NULL,
    PRIMARY KEY (RaceID),
);
...
ALTER TABLE Race ADD
    FOREIGN KEY (fk_Result1ID) REFERENCES FirstPlace(Result1ID),
    FOREIGN KEY (fk_Result2ID) REFERENCES SecondPlace(Result2ID),
    FOREIGN KEY (fk_Result3ID) REFERENCES ThirdPlace(Result3ID);
...
--- Test to see if all the tables were created right
select * from Driver;
select * from Race;
select * from Team;
